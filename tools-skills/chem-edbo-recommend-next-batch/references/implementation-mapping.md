...
| 8 | 批推荐：联合 EI 优化 | 贪心 + Kriging-believer 幻想更新（预测均值作为伪观测，重拟合后选下一个） | `recommend()` 批循环 | **近似**：顺序批 EI，非联合 qEI；批内多样性略低（缓解见 SKILL.md 故障排查） |
...
| 12 | 置信度/不确定性报告 | 每推荐附 predicted_std 与置信度分档（0.5σ/1.5σ 观测标准差） | `_make_rec()` | 分档阈值自定义（低可信度项，可调） |
...
2. 无安全域约束（除 bounds 告警外不限制搜索，GP 可预测出物理界限外的值）；
...
- `schema_version: "1.0"`（输出 JSON 中声明）；改动推荐结构时须递增并更新 SKILL.md。
