...
Each applicability rule, precondition, input, output, step, decision branch, verification check, success criterion, stop condition and requirement needs precise citations: source_id, original 1-based start/end lines, and an exact short quote from those lines.
...
For every candidate, state at least one observable, source-supported success criterion: the documented return status, output property, or acceptance condition that shows the requested operation completed. A successful call does not by itself establish a broader scientific claim.
